...
- **Performance: deferred non-critical init**: History rendering, cheat sheet, and SW registration wrapped in `requestIdleCallback` (with `setTimeout` fallback) so main UI renders before secondary features load
- SW cache bumped to `uniswap-v26`
...
- SW cache bumped to `uniswap-v25`
...
- SW cache bumped to `uniswap-v24`
